...
std::string GetDeviceLibrariesVersionInfo(const std::string& csv_content)
{
    ...
    std::string device_libraries_build_string =
        absl::StrFormat("%s-%s-%s-%s", device_info_map[kNameVersion],
                        device_info_map[kNameReleaseType], device_info_map[kNameAbi], short_sha);

    return absl::StrFormat(
        "Device Libraries Build: %s\nDevice Libraries Build Type: %s\nDevice "
        "Libraries SHA: %s\n",
        device_libraries_build_string, device_info_map[kNameBuildType],
...
std::string GetLongVersionString()
{
    ...
    if (absl::StatusOr<std::string> ret =
            ReadFileCapped(device_libraries_version_path, kMaxCharactersDeviceLibraryFile);
        ret.ok())
    {
