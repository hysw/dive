...
static bool IsDiveContainerFormat(std::filesystem::path filename, bool check_dive_magic = true)
{
...
std::optional<std::filesystem::path> ExtractDiveArchiveTo(std::filesystem::path  out_path,
                                                          ...
                                                          DiveVirtualFilesystem& vfs)
{
    ...
    while (reader.Next())
    {
        ...
        auto file_info = DiveVirtualFilesystem::File{
            ...
            .m_extract_path = out_path / GetExtractedName(name),
        };
...
void DiveVirtualFilesystem::Reset()
{
    m_files.clear();
}
...
bool DiveFileContainer::Load(std::filesystem::path source)
{
    ...
    if (source.extension() == ".dive_tar" && IsDiveContainerFormat(source, false))
    {
        CreateTempDirectory();
        auto main_file_path = ExtractDiveArchiveTo(m_temp_dir, source, m_vfs);
        if (main_file_path)
        {
...
void DiveFileContainer::CreateTempDirectory()
{
    if (!m_temp_dir.empty())
    {
        return;
...
bool DiveFileContainer::Save(std::filesystem::path target)
{
    ...
    auto temp_save_target = m_temp_dir / "dive-save.tar";
